...
const ListeningExercise: React.FC = () => {
	...
	const checkAnswer = useCallback(
		(answer: string) => {
			return () => {
				...
				if (state.currentExercise[state.exerciseIndex]?.correctAnswer === answer) {
	...
	useEffect(() => {
		const shuffledExercises = initialExercises.map((exercise) => ({
			...exercise,
	...
	const { exerciseIndex, currentExercise, progress, gameStarted, hasSelectedAnswer, selectedAnswer } = state;
	...
	return (
		<Layout>
			...
			{gameStarted && currentExercise[exerciseIndex] && renderExerciseScreen()}
			{!currentExercise[exerciseIndex] && renderCompletionScreen()}
		</Layout>
	);
};
