...
const LessonPage: React.FC = () => {
	...
	const checkLessonExistence = async (lessonNum: number) => {
		try {
			await import(`../../../lessons/${user?.learning_language?.name.toLowerCase()}/${user?.native_language_code}/lesson${lessonNum}.mdx`);
			return true;
		} catch (error) {
			return false;
		}
	...
	useEffect(() => {
		const fetchFileContent = async () => {
			try {
				const lessonModule = await import(`../../../lessons/${user?.learning_language?.name.toLowerCase()}/${user?.native_language_code}/lesson${lesson_number}.mdx`);
